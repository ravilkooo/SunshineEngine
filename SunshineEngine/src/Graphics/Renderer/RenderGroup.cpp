...
namespace SE_G {
	...
	void RenderGroup::DisableAllTechniques(SE::UUID uuid)
	{
		for (auto& pass : m_passes)
		{
			pass.second->DisableTechnique(uuid);
		}
	}

	void RenderGroup::EnableAllTechniques(SE::UUID uuid)
	{
		for (auto& pass : m_passes)
		{
			pass.second->EnableTechnique(uuid);
		}
	}
