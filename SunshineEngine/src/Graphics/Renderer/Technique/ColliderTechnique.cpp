...
namespace SE_G {
	...
	void ColliderTechnique::Pass(ID3D11DeviceContext* context)
	{
		BindAll(context);
		DrawTechnique(context);
	}

	void ColliderTechnique::BindAll(ID3D11DeviceContext* context)
	{
		UpdateTransformCB(context);
		UpdateSettingsCB(context);

		m_transformCB->Bind(context);
		m_settingsCB->Bind(context);

		switch (m_colliderData->m_shapeType)
		{
		case SE::ColliderShapeType::Box:
			s_boxShader->Bind(context);
			break;

		case SE::ColliderShapeType::Sphere:
			s_sphereShader->Bind(context);
			break;

		case SE::ColliderShapeType::Capsule:
			s_capsuleShader->Bind(context);
			break;

		case SE::ColliderShapeType::TaperedCapsule:
			s_taperedCapsuleShader->Bind(context);
			break;

		case SE::ColliderShapeType::Mesh:
			//m_customVertexBuffer->Bind(context);
			s_customShader->Bind(context);
			break;
	...
	void ColliderTechnique::DrawTechnique(ID3D11DeviceContext* context)
	{
		switch (m_colliderData->m_shapeType)
		{
		case SE::ColliderShapeType::Box:
	...
	void ColliderTechnique::UpdateTransformCB(ID3D11DeviceContext* context)
	{
		m_transformCB->Update(context, { m_colliderData->m_transformMat });
	}

	void ColliderTechnique::UpdateSettingsCB(ID3D11DeviceContext* context)
	{
		m_settingsCB->Update(context, m_colliderData->m_settings);
	}
