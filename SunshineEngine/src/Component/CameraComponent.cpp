...
void CameraComponent::FromJson(const json& j)
{
	...
	if (j.contains("m_springArmParams"))
	{
		if (j["m_springArmParams"].contains("length")) {
			m_camera->SetSpringArmLength(j["m_springArmParams"]["length"].get<float>());
		}
		if (j["m_springArmParams"].contains("pitchYawRoll") && j["m_springArmParams"]["pitchYawRoll"].is_array() && j["m_springArmParams"]["pitchYawRoll"].size() >= 3) {
			m_camera->SetSpringArmRotation(DXSM::Vector3(
				j["m_springArmParams"]["pitchYawRoll"][0].get<float>(),
				j["m_springArmParams"]["pitchYawRoll"][1].get<float>(),
				j["m_springArmParams"]["pitchYawRoll"][2].get<float>()
			));
		}
		if (j["m_springArmParams"].contains("rootOffset") && j["m_springArmParams"]["rootOffset"].is_array() && j["m_springArmParams"]["rootOffset"].size() >= 3) {
			m_camera->SetSpringArmRootOffset(DXSM::Vector3(
				j["m_springArmParams"]["rootOffset"][0].get<float>(),
				j["m_springArmParams"]["rootOffset"][1].get<float>(),
				j["m_springArmParams"]["rootOffset"][2].get<float>()
			));
		}
	}
	if (j.contains("cameraPitchYawRoll") && j["cameraPitchYawRoll"].is_array() && j["cameraPitchYawRoll"].size() >= 3) {
		m_camera->SetCameraRotation(DXSM::Vector3(
			j["cameraPitchYawRoll"][0].get<float>(),
			j["cameraPitchYawRoll"][1].get<float>(),
			j["cameraPitchYawRoll"][2].get<float>()
		));
	}
	if (j.contains("m_zoomAccelerations")) {
		m_camera->SetZoomAcceleration(j["m_zoomAccelerations"].get<float>());
	}
}
...
{
	json j = json::object();
	auto temp = m_assignedComponent->m_camera->GetCameraRotation();
	j["cameraPitchYawRoll"] = { temp.x, temp.y, temp.z };

	j["m_springArmParams"] = json::object();
	j["m_springArmParams"]["length"] = m_assignedComponent->m_camera->GetSpringArmLength();
	temp = m_assignedComponent->m_camera->GetSpringArmRotation();
	j["m_springArmParams"]["pitchYawRoll"] = { temp.x, temp.y, temp.z };
	temp = m_assignedComponent->m_camera->GetSpringArmRootOffset();
	j["m_springArmParams"]["rootOffset"] = { temp.x, temp.y, temp.z };
	j["m_zoomAccelerations"] = m_assignedComponent->m_camera->GetZoomAcceleration();
