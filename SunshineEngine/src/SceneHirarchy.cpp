...
SceneGraph::SceneGraph(std::unordered_map<SE::UUID, eastl::unique_ptr<GameObject_Info>>& uuidToObjectMap)
...
void SceneGraph::Attach(SE::UUID childUUID, SE::UUID newParentUUID) {
...
SE::UUID SceneGraph::Add(SE::UUID objUUID)
{
...
void SceneGraph::Reparent(SE::UUID objUUID, SE::UUID newParent) {
    Attach(objUUID, newParent);
}

void SceneGraph::EraseSubtree(SE::UUID nodeUUID) {
